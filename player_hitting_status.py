...
hitters = batting_stats(2024, qual=50)
...
sns.scatterplot(data=nonstl_hitters, x=x_axis, y=y_axis, color='gold', alpha=1, size="AB")  # Create scatter plot using seaborn
...
for index, row in stl_hitters.iterrows():
    plt.text(row[x_axis], row[y_axis] + .0125, row["Name"], fontsize=5, ha='center', va='bottom', fontfamily='roboto',bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.2'))

for index, row in nonstl_hitters.iterrows():
     plt.text(row[x_axis], row[y_axis] + .0125, row["Name"], fontsize=6, ha='center', va='bottom', fontfamily='roboto',bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.2'))
